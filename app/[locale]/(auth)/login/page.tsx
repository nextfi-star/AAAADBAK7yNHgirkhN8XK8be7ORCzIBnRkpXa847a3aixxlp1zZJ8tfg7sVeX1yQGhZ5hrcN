	...
	const onSubmit = async (data: any) => {
		...
		try {
			...
			if (response.response === 'success') {
				...
				useThemeStore.getState().setUser({ email, phone, username, uid, password });
		...
		<div className='form__wrapper flex flex-col justify-between gap-[20px]'>
			...
			<form onSubmit={handleSubmit(onSubmit)} className='form'>
				<div className='w-full'>
					...
					/>
					{errors.emailOrPhone && <p className='absolute text-danger pointer-events-none'>{errors.emailOrPhone.message}</p>}
				</div>
				<div className='password__wrapper relative w-full'>
					...
					{errors.password && <p className='absolute text-danger pointer-events-none'>{errors.password.message}</p>}
				</div>
				...
				{error && <p className='text-danger pointer-events-none'>{error}</p>}
				<button
					type='submit'
