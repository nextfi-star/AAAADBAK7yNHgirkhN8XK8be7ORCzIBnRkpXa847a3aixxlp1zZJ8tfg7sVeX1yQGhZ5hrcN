import React from 'react'
const data = [
	//! Требуется дополнительная разработка объекта для дин. отображения
	{
		name: 'Transfer',
		id: 1,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 122239745679182,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 1837461894657,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 14,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 145,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 51,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 15,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 441,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 123123123123,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 213123132,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 71231231231231,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 172222,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 11231231231238,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 881123123123,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 16,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 143,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 221,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 111,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 1123,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 1766,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 5441,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 4441,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 2324122,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 23423441,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 2323423423423241,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 1234571,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 987651,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 11245677,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 134563456,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 34634631,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 3463461,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 23436781,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 1456456345,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 34535645671,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 14545645,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 1456456456345,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 123423241,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 2345361,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
	{
		name: 'Transfer',
		id: 346578781,
		cell: (
			<span className='flex items-center gap-[5px] text-[14px] dark:text-white text-[#0c0c0c]'>
				08/07/24,{' '}
				<span className='text-[14px] dark:text-white text-[#0c0c0c]'>
					14:55:39
				</span>
			</span>
		),
		symbol: '+',
		money: '+112 USDT',
	},
]
const Transaction = () => {
	return (
		<section className='dark:!bg-[#1e1e1e66] bg-white shadow-medium rounded-[30px] p-[27px_40px] flex flex-col gap-[20px] min-h-[690px] max-h-[690px] overflow-y-auto'>
			<h3 className='text-[14px] md:text-[20px] 2xl:text-[25px] w-full'>
				Recent Transactions
			</h3>
			<div className='flex flex-col gap-[10px] max-h-[690px] overflow-y-auto'>
				{data &&
					data.map(item => (
						<div key={item.id}>
							<div className='flex items-center w-full justify-between'>
								<div className='flex flex-col gap-[5px] items-start'>
									<h5 className='text-[14px] md:text-[20px] 2xl:text-[23px]'>
										{item.name}
										{item.cell}
									</h5>
								</div>
								<span className='text-green-700 text-[14px] 2xl:text-[20px]'>
									{item.money}
								</span>
							</div>
							<span className='min-h-[1px] w-full bg-white block' />
						</div>
					))}
			</div>
		</section>
	)
}

export default Transaction
